...
/*
 * 4. Model 객체를 이용한 파라미터 처리  => forward 할 때 데이터 저장용으로 사용한다.
 */

@Controller
public class MyController4 {

  //method=RequestMethod.GET => GET 방식, 디폴트로 생략 가능
  // value 만 남으면 value 도 생략 가능  
  ...
  // 파라미터명과 필드명과 동일하면 알아서 setter를 호출함
  // @RequestMapping(value="/blog/add.do", method=RequestMethod.POST)
  public String add(BlogDto blog) { // 커맨드 객체의 Model 저장 방식 : 클래스 타입을 camelCase로 변경해서 저장한다. (BlogDto -> blogDto로 변경해서 저장)
