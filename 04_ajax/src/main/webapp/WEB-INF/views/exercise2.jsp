...
<!--  부트 스트랩 사용을 위한 CDN (모달창 사용을 위해 추가해야 함) -->
...
<body>
  ...
  <div>
     <button type="button" class="btn btn-primary" onclick="fnBoardList()">목록갱신</button> <%-- 인라인 이벤트 방식 --%>
  </div>
 ...
 <!----------------------------------------------------------------------------------->
    ...
    const fnBoardList = ()=>{
    	$.ajax({
    		...
    		$.each(resData, (i, board)=>{
    			result += '<div class="board" data-board-no="'+ board.boardNo +'"><div>' + board.boardNo + '</div><div>' + board.title + '</div><div>' + board.content + '</div></div>';
    		})
    		  result += '</div>';
  ...
  <script>
    ...
    const fnBoardDetail = ()=>{
      ...
      $(document).on('click', '.board', (evt)=>{            // .on(이벤트방식, 이벤트대상, 이벤트리스너)
